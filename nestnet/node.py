...
class Isula(Host):
    ...
    def terminate(self):
        ...
        if not isula.is_container_running(self.did):
            return
        try:
            isula.remove_container(self.dc)
    ...
    def _check_shell(self):
        ...
        if isula.is_container_running(self.did):
            if self.shell:
                self.shell.poll()
    ...
    def updateCpuLimit(self, cpu_quota=-1, cpu_period=-1, cpu_shares=-1, cores=None):
        ...
        if cores:
            self.resources['cpuset_cpus'] = cores
            isula.update_resourse(self.dc)
