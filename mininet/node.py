...
import signal
import select
from mininet.isula import isula  # Need to build
import json
...
class Isula(Host):
    """Node that represents a isula container.
    We use the isula-py(building) client library to control isula.
    """

    def __init__(self, name, config,
                 **kwargs):
        """
        Creates a iSula container as Mininet host.
        ...
        * updateMemoryLimits(...)
        ...
        self.resources = dict(
            ...
            cpuset_mems=defaults['cpuset_mems'],
        ...
        # setting PS1 at "isula run" may break the python isula api (update_container hangs...)
        ...
        # delete container that has same name
        ...
        #     #network_disabled=True,  # isula stats breaks if we disable the default network
        #     host_config=hc,
        ...
        # debug("iSula container %s started\n" % name)
    ...
    def start(self):
        ...
        if cmd_field is not None:
            cmd_field.append("> /dev/pts/0 2>&1")  # make output available to isula logs
    ...
    def popen(self, *args, **kwargs):
        """Return a Popen() object in node's namespace
           ...
           kwargs: Popen() keyword args"""
        if not self._is_container_running():
            error("ERROR: Can't connect to Container \'%s\'' for isula host \'%s\'!\n" % (self.did, self.name))
            return
        mncmd = ["isula", "exec", "-t", "%s.%s" % (self.dnameprefix, self.name)]
    ...
    def update_resources(self, resource):
        """
        Update the container's resources using the isula.update function
        ...
        isula.update_resourse(self.resources)
    ...
    def updateCpuLimit(self, cpu_quota=-1, cpu_period=-1, cpu_shares=-1, cores=None):
        ...
        This method allows to update resource limitations at runtime by bypassing the iSula API
    ...
    def cgroupSet(self, param, value, resource='cpu'):
        """
        Directly manipulate the resource settings of the iSula container's cgrpup.
        Args:
